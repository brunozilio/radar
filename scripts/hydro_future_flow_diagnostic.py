...
OUT=ROOT/'outputs/diagnostico-vazoes-conhecidas-depois-exato-20260921'
...
def run():
    protocol=ROOT/'docs/future-flow-exact-diagnostic-protocol.json'
    ...
    source_manifest=ROOT/'outputs/auditoria-consistencia-componentes-ceran-20260921/source-manifest.json'
    sources=json.loads(source_manifest.read_text());paths.append(source_manifest)
    paths.extend(ROOT/r['path'] for r in sources)
    hashes={str(p):hashlib.sha256(p.read_bytes()).hexdigest() for p in paths}
    for r in sources:assert hashes[str(ROOT/r['path'])]==r['sha256']
    ...
    # ONS exact timestamps are required; do not relabel an as-of value as exact.
    raw_julho={}
    for source in sources:
        with (ROOT/source['path']).open() as f:
            for line,row in enumerate(csv.DictReader(f,delimiter=';'),2):
                if row['id_reservatorio'].strip()!='JIUHQJ':continue
                at=epoch(row['din_instante']);assert at not in raw_julho
                try:q=float(row['val_vazaodefluente'])
                except (TypeError,ValueError):q=np.nan
                raw_julho[at]=(q,source['path'],line)
    truth={'julho':np.full(len(t),np.nan),'carreiro':d['carreiro']*1000};audit=[]
    np.testing.assert_allclose(truth['carreiro'],z['raw:86500000:Q'][np.searchsorted(z['times'],t)],rtol=0,atol=1e-9,equal_nan=True)
    for i,at in enumerate(t):
        q,path,line=raw_julho.get(at,(np.nan,'',None));frozen=d['julho'][i]*1000
        status='missing_exact_ons_record' if at not in raw_julho else 'nonfinite_exact_q' if not np.isfinite(q) else 'baseline_disagreement' if not np.isfinite(frozen) or abs(q-frozen)>1e-7 else 'exact_and_consistent'
        if status=='exact_and_consistent':truth['julho'][i]=q
        if epoch('2026-07-01T00:00:00-03:00')<=at<epoch('2026-09-21T00:00:00-03:00'):
            audit.append(dict(epoch=float(at),status=status,raw_ons_q_m3_s=float(q) if np.isfinite(q) else None,
                              baseline_q_m3_s=float(frozen) if np.isfinite(frozen) else None,source_path=path,source_line=line))
    ...
    for r in csv.DictReader(paths[1].open()):
        ...
        deltas={s:observed_delta(flows[s][i],truth[s],i,h,*kernels[s]) for s in ('julho','carreiro')}
    ...
    savecsv(OUT/'julho-exact-observation-audit.csv',audit)
    np.savez_compressed(OUT/'exact-diagnostic-observations.npz',times=t,**truth)
