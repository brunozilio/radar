"""Prepare two independent 2018 observed Radar matrices; no fit or inference."""
import csv
import hashlib
import json
...
OUT = ROOT / 'outputs/radar-matrizes-observadas-2018-20260922'
HYDRO = ROOT / 'outputs/radar-insumos-observados-2018-20260922'
QI = ROOT / 'outputs/auditoria-qi-radar-2018-20260922'
PROTOCOL = ROOT / 'docs/radar-2018-observed-features-protocol.json'
...
def run():
    ...
    paths.update(HYDRO / 'stations' / w['warmup_start'] / f'ana-{code}-all-qc.jsonl' for w in protocol['windows'] for code in delays)
    ons = []
    ...
    for window in protocol['windows']:
        ...
        for code in sorted(delays):
            rows = [json.loads(line) for line in (HYDRO / 'stations' / window['warmup_start'] / f'ana-{code}-all-qc.jsonl').read_text().splitlines()]
        ...
        assert features.shape == (168, 120) and np.isnan(features[:, 1]).all() and np.isnan(features[:, 12:24]).all()
