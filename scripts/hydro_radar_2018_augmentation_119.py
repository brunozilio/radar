...
OUT=ROOT/'outputs/experimento-radar-historico-2018-119-20260922'
RECENT=ROOT/'outputs/radar-matriz-recente-ancora-horaria-20260922'
OLD=ROOT/'outputs/radar-matrizes-observadas-2018-20260922'
PROTOCOL=ROOT/'docs/radar-2018-hourly-augmentation-119-protocol.json'
...
def run():
    ...
    upstream_complete=np.isfinite(X[:,6:24]).all(axis=1)
    feature_indices=np.delete(np.arange(120),1)
    X=X[:,feature_indices]
    old=[{**d,'features':d['features'][:,feature_indices]} for d in old]
    ...
    np.save(OUT/'feature-indices.npy',feature_indices)
    ...
    prior=ROOT/'outputs/experimento-radar-historico-2018-20260922'
    for file,expected in [('training-masks.npz',masks),('training-vectors.npz',vectors)]:
        frozen=np.load(prior/file)
        assert set(frozen.files)==set(expected)
        for key,value in expected.items(): np.testing.assert_array_equal(value,frozen[key])
        hashes[str((prior/file).resolve())]=sha(prior/file)
    assert list(csv.DictReader((prior/'training-plan.csv').open()))==list(csv.DictReader((OUT/'training-plan.csv').open()))
    hashes[str((prior/'training-plan.csv').resolve())]=sha(prior/'training-plan.csv')
    ...
    for h in range(1,13):
        for phase in ('validation','test'):
            ...
            for i in scheduled:
                rows.append({'phase':phase,'origin':iso(t[i]),'target_time':iso(t[i]+h*3600),'horizon_h':h,'complete_upstream18':bool(upstream_complete[i]),'base_m':number(b[i]),'actual_m':number(target[i]),**{family+'_m':number(predictions[family][i]) for family in FAMILIES}})
    ...
    dump(OUT/'experiment.json',{'completed_at_utc':datetime.now(timezone.utc).isoformat(),'input_sha256':hashes,'prepared_sha256':sealed,'runtime':runtime,'models_fitted':48,'estimator_features':119,'source_features':120,'dropped_source_columns':[1],'rows':len(rows),'metrics':len(metrics),'promoted':False,'goal_achieved':False})
