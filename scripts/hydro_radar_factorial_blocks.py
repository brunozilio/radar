"""Complete the predeclared RADAR gauge/rain/year factorial input comparison."""
import csv
...
def run():
    ...
    core = ROOT/'outputs/experimento-radar-core33-20260921'
    protocol = ROOT/'docs/radar-factorial-blocks-protocol.json'
    spec = json.loads(protocol.read_text())
    columns={name:np.array(values) for name,values in spec['new_columns_zero_based'].items()}
    np.testing.assert_array_equal(columns['levels45'],np.arange(45))
    np.testing.assert_array_equal(columns['rain108'],np.r_[np.arange(12),np.arange(24,120)])
    corecols=np.r_[np.arange(12),np.arange(24,45)]
    paths = [protocol,verified(native,'features.npz'),verified(new,'features.npz'),
             verified(core,'predictions.csv'),verified(core,'training-masks.npz'),
             verified(ref,'training.csv'),verified(core,'experiment.json'),
    ...
    assert runtime==json.loads((core/'experiment.json').read_text())['runtime']
    ...
    for phase in ('validation','test'):
        for family in ('core_original','core_augmented'):
            for h in range(1,13): paths.append(verified(core,f'models/{phase}-{family}-{h}.joblib'))
    hashes={str(p.resolve()):sha(p) for p in paths}
    out=ROOT/'outputs/experimento-radar-factorial-blocos-20260921'
    ...
    for h in range(1,13):
        ...
        for phase,left,right in [('validation',start,end),('test',end,stop)]:
            ...
            for family in ('core_original','core_augmented'):
                control=joblib.load(core/f'models/{phase}-{family}-{h}.joblib')
                actual=control.predict(F[apply][:,corecols])+base[apply]
                expected=np.array([float(lookup[phase,iso(t[i]),h][family+'_m']) for i in apply])
                np.testing.assert_array_equal(actual,expected)
                reproduction.append(dict(phase=phase,horizon_h=h,family=family,n=len(apply),max_difference_m=0.0))
            for family,include,group in [(g+'_'+suffix,inc,g) for g in ('levels45','rain108') for suffix,inc in [('original',False),('augmented',True)]]:
                CF=F[:,columns[group]];NCF=NF[:,columns[group]]
                ...
                training.append(dict(phase=phase,horizon_h=h,family=family,original_n=len(train),
                    added_n=len(newidx) if include else 0,n=len(yfit),features=CF.shape[1],
            ...
            for i in scheduled:
                ...
                assert all(bool(r['observed_control_m'])==bool(np.isfinite(pred[i])) for pred in values.values())
                row=dict(phase=phase,origin=r['origin'],target_time=r['target_time'],nominal_lead_h=h,
                    ...
                    core_original_m=float(r['core_original_m']) if r['core_original_m'] else None,
                    core_augmented_m=float(r['core_augmented_m']) if r['core_augmented_m'] else None)
                for family,pred in values.items(): row[family+'_m']=float(pred[i]) if np.isfinite(pred[i]) else None
                predictions.append(row)
        print('completed RADAR gauge/rain/year factorial horizon',h,flush=True)
    ...
    summary=dict(input_sha256=hashes,runtime=runtime,models_fitted=96,control_models_refitted=0,
        control_models_reproduced=96,rows=len(predictions),identical_availability=True,
