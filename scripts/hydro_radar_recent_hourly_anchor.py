"""Prepare recent research predictors from audited strict-QC station archives."""
...
OUT = ROOT / 'outputs/radar-matriz-recente-ancora-horaria-20260922'
PROTOCOL = ROOT / 'docs/radar-recent-hourly-anchor-features-protocol.json'
...
def run(strict_dir, audit_manifest):
    ...
    for code, path in station_paths.items():
        ...
        if code in LEVELS:
            ...
            if code == '86510000':
                whole = t % 3600 == 0
                delayed[code + ':H'] = asof(t[whole],h[whole],grid-lag,max_age=0)
            else:
    ...
    summary = {'origins':len(times),'features':120,'finite_base':int(np.isfinite(base).sum()),'finite_truth':int(np.isfinite(truth).sum()),'missing_cells':int(np.isnan(X).sum()),'complete24':0,'fits':0,'inferences':0,'operational_changes':0,'goal_achieved':False}
