...
for s in selected:
 for i in range(28):
  d=datetime.date(2026,7,25)+datetime.timedelta(days=i)
...
(R/'archive-events-fetch-manifest.json').write_text(json.dumps(results,indent=2));print('Complete',len(results),'errors',sum('error' in r for r in results),flush=True)
