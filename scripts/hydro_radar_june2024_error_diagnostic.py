...
SOURCE=ROOT/'outputs/experimento-radar-historico-junho2024-20260922'
MATRIX=ROOT/'outputs/radar-matriz-recente-ancora-15min-20260922/features.npz'
OUT=ROOT/'outputs/diagnostico-erros-radar-junho2024-20260922'
...
def run():
    ...
    for r in csv.DictReader((SOURCE/'predictions.csv').open()):
        ...
        actual=numeric(r['actual_m']);control=numeric(r['native_control_m']);aug=numeric(r['augmented_m']);base=numeric(r['base_m'])
        ...
        complete=bool(np.isfinite(d['features'][i,6:24]).all());assert bool(np.isfinite(d['features'][i,:24]).all())==(r['original_complete24']=='True')
        ...
        rows.append({'phase':r['phase'],'origin':r['origin'],'target_time':r['target_time'],'horizon_h':int(r['nominal_lead_h']),
    ...
    for (phase,h),all_rows in sorted(groups.items()):
        for subset in ('all','level_ge_7m'):
            ...
            total=summarize(rr)
            for family,key in [('native_control','control_hits'),('augmented','augmented_hits')]:
                old=original[phase,h,subset,family]
                assert total['observed']==int(old['observed_targets']) and total['pairs']==int(old['n']) and total['failures']==int(old['failures']) and total[key]==int(old['hits'])
                checks+=4
            ...
            if subset=='level_ge_7m':
                dates=defaultdict(list)
                for r in rr:dates[r['target_time'][:10]].append(r)
                for day,group in sorted(dates.items()):days.append({'phase':phase,'horizon_h':h,'target_date':day,**summarize(group)})
    save(OUT/'strata.csv',metrics);save(OUT/'target-days.csv',days)
    for name,digest in hashes.items():assert sha(ROOT/name)==digest
    dump(OUT/'verification.json',{'rows':len(rows),'metrics':len(metrics),'horizon_calendar_date_groups':len(days),'checks':checks,'fits':0,'inferences':0,'promoted':False,'goal_achieved':False})
