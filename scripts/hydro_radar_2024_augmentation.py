"""Frozen RADAR native-missing control versus predeclared 2024 data augmentation."""
import csv
...
def run():
    ...
    protocol = ROOT/'docs/radar-2024-augmentation-protocol.json'
    paths = [protocol, verified(ref, 'features.npz'), verified(ref, 'predictions.csv'),
    ...
    assert F.shape == (len(t),180) and NF.shape == (744,180)
    assert np.all(np.diff(t)==3600) and np.all(np.diff(nt)==3600) and nt.max()<t.min()
    assert not added['complete24'].any() and np.isnan(NF[:,18:24]).all()
    ...
    meta = {(r['phase'],int(r['horizon_h'])):r for r in csv.DictReader(paths[3].open()) if r['family']=='candidate'}
    ...
    out = ROOT/'outputs/experimento-radar-historico-2024-20260921'
    ...
    assert nt[0]==epoch('2024-04-01T00:00:00-03:00') and nt[-1]==boundary-3600
    ...
    for h in range(1,13):
        ...
        for phase,left,right in [('validation',start,end),('test',end,stop)]:
            ...
            frozen_values = np.array([float(lookup[phase,iso(t[i]),h]['candidate_m']) for i in apply])
            ...
            training.append(dict(phase=phase,horizon_h=h,original_n=len(train),added_n=len(newidx),n=len(yfit),
                features=180,original_targets_ge_7m=int((target[train]>=7).sum()),
        ...
        print('completed RADAR 2024 augmentation horizon',h,flush=True)
