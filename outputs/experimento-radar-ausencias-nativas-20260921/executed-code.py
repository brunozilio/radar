...
def run():
    protocol=ROOT/'docs/radar-native-missing-protocol.json'
    ...
    out=ROOT/'outputs/experimento-radar-ausencias-nativas-20260921';out.mkdir(exist_ok=False);(out/'models').mkdir()
    predictions=[];training=[];max_reproduction=0.;reproduced=0
    for h in range(1,13):
                ...
                if split=='test' and family=='baseline':
                    for i in scheduled:
                        ...
                        if key in old:
                            ...
                            if error>1e-8:raise ValueError(f'Historical Radar control differs at {key}: {error}')
                values[family]=pred
    ...
    summary=dict(input_sha256=hashes,rows=len(predictions),models_fitted=48,
                 historical_test_predictions_reproduced=reproduced,max_reproduction_error_m=max_reproduction,
