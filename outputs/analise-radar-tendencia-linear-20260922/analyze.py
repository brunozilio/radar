...
def main():
    ...
    contrasts=[]
    for n in metrics:
        if n['family']!='linear_trend_hybrid':continue
        for ref in ('mixed_profile_candidate','profile_A_control','profile_B_control'):
            ...
            c=dict(phase=n['phase'],profile=n['profile'],horizon_h=int(n['horizon_h']),population=n['population'],subset=n['subset'],reference=ref,
                   relation='primary_tree_only' if ref=='mixed_profile_candidate' else 'matching_profile' if ref==f"profile_{n['profile']}_control" else 'cross_profile',
                   observed_targets=int(n['observed_targets']),pairs=int(n['pairs']),failures=int(n['failures']),reference_hits=int(r['hits']),hybrid_hits=int(n['hits']),hits_delta=int(n['hits'])-int(r['hits']))
            for field in ('mae_m','bias_m','p98_abs_m','max_abs_m'):
                c['reference_'+field]=float(r[field]) if r[field] else None;c['hybrid_'+field]=float(n[field]) if n[field] else None
    ...
    lines=['# Tendências lineares com árvores dos resíduos','',
           ...
           '|---|---|---:|---:|---:|---:|---:|']
    for r in contrasts:
        if r['relation']=='primary_tree_only' and r['population']=='full_schedule' and r['subset']=='level_ge_7m' and r['horizon_h'] in (1,6,12):
            control=lookup[r['phase'],r['profile'],str(r['horizon_h']),'full_schedule','level_ge_7m',f"profile_{r['profile']}_control"]
            lines.append(f"| {r['phase']} | {r['profile']} | {r['horizon_h']} | {r['reference_hits']} → {r['hybrid_hits']} / {r['observed_targets']} | {control['hits']} | {r['reference_mae_m']:.4f} → {r['hybrid_mae_m']:.4f} | {r['reference_max_abs_m']:.4f} → {r['hybrid_max_abs_m']:.4f} |")
    lines+=['','Comparações completas contra as árvores da mistura, contra o controle do perfil e contra o controle cruzado estão no CSV. Melhorar a mistura anterior não implica superar o controle dedicado. Não selecionar perfis ou horizontes favoráveis. Nenhuma promoção ou comprovação da meta de 98%.','']
