...
Main models stay frozen. An explicitly selected policy can fit pre-cutoff
Carreiro proxies for missing inputs; no live issuance or promotion occurs.
Historical availability assumptions and exclusions remain explicit. Future
upstream observations never select origins.
"""
...
from hydro_hourly_forecast import ROOT, BASE, epoch, iso, dump, ridge_fit
...
def fill_missing(original, estimates):
    original = np.asarray(original); estimates = np.asarray(estimates)
    if original.shape != estimates.shape:
        raise ValueError('Replacement shape mismatch')
    return np.where(np.isfinite(original), original, estimates)


def fit_regional_proxy(features, times, truth, lead, cutoff):
    # Deliberately exclude all four CarreiroQ predictors; no recursive estimates.
    X = np.delete(features, np.arange(24, 28), axis=1)
    target = shift(truth, -lead)
    train = np.where(np.isfinite(target) & (times+lead*3600 < cutoff))[0]
    if not len(train) or np.any(times[train]+lead*3600 >= cutoff):
        raise ValueError('Invalid proxy training boundary')
    model = ridge_fit(X, target, train, 1000., 1+2*(target > 2))
    return model, np.maximum(predict_many(model, X), 0)*1000, train


def route(history, forecast, weights, lags, origin_index, horizon, *, skip_zero_weights=False):
    ...
    for weight, lag in zip(weights, lags):
        if skip_zero_weights and weight == 0:
            continue
        k = horizon-lag
        if k >= 0:
            value = forecast[k]
...
def anchor_delay_hours(minutes):
    # Only interior quarter-hour anchors are supported by this interpolation.
    # Longer delays need earlier routed states, not extrapolation of this pair.
    if isinstance(minutes, bool) or minutes not in (15, 30, 45):
        raise ValueError('Anchor delay must be 15, 30 or 45 minutes')
    return minutes / 60.


def reconstruct_anchor(previous_q, origin_q, delay_h):
    if not 0 < delay_h < 1:
        raise ValueError('Anchor must lie strictly inside the previous hour')
    return delay_h*previous_q + (1-delay_h)*origin_q


def residual_correction(residual, elapsed_hours, tau_hours=6):
    if isinstance(tau_hours, bool) or tau_hours not in (2, 6, 12):
        raise ValueError('Supported correction time constants are 2, 6 and 12 hours')
    if not np.isfinite(elapsed_hours) or elapsed_hours < 0:
        raise ValueError('Elapsed time from the anchor must be nonnegative')
    return residual*np.exp(-elapsed_hours/tau_hours)
...
def run(out, protocol_path):
    ...
    delay_minutes = protocol.get('anchor_delay_minutes', 15)
    delay_h = anchor_delay_hours(delay_minutes)
    tau_hours = protocol.get('correction_tau_hours', 6)
    residual_correction(0., 0., tau_hours)  # Validate before creating outputs.
    skip_zero = protocol.get('skip_exact_zero_routing_weights', False)
    if not isinstance(skip_zero, bool):
        raise ValueError('Zero-weight routing policy must be boolean')
    missing_policy = protocol.get('missing_flow_policy', 'none')
    if missing_policy not in ['none', 'regional_ridge']:
        raise ValueError('Unsupported missing flow policy')
    ...
    proxy = None; proxy_models = {}; proxy_training = []
    original_car_future = f['carreiro', 'reference'].copy()
    if missing_policy == 'regional_ridge':
        proxy_cutoff = epoch('2026-07-01T00:00:00-03:00'); values = []
        for lead in range(12):
            model, estimate, train = fit_regional_proxy(F, t, d['carreiro'], lead, proxy_cutoff)
            values.append(estimate)
            proxy_models[str(lead)] = {k: v.tolist() if isinstance(v, np.ndarray) else float(v) for k, v in model.items()}
            proxy_training.append({'lead_h': lead, 'training_n': len(train),
                                   'latest_training_target': iso((t[train]+lead*3600).max()),
                                   'cutoff_exclusive': iso(proxy_cutoff)})
        proxy = np.column_stack(values)
        f['carreiro', 'reference'] = fill_missing(original_car_future, proxy)
        dump(out/'proxy-models.json', proxy_models); savecsv(out/'proxy-training.csv', proxy_training)
        np.savez_compressed(out/'modeled-carreiro-inputs.npz', times=t, estimated_q_m3_s=proxy,
                            historical_missing=~np.isfinite(d['carreiro']), future_missing=~np.isfinite(original_car_future))
    ...
    original_car_history = upstream_history['carreiro'].copy()
    if proxy is not None:
        upstream_history['carreiro'] = fill_missing(original_car_history, proxy[:, 0])
    for sequence, i in enumerate(origins):
        ai = anchor_index(z['times'], t[i], delay_minutes*60); anchor_h = z['raw:86510000:H'][ai] if ai is not None else np.nan
        ...
        def routed(h, family):
            return sum(route(upstream_history[s], f[s, family if s == 'julho' else 'reference'][i], weights[s],
                             range(1, 13) if s == 'julho' else range(4, 25), i, h,
                             skip_zero_weights=skip_zero) for s in ['julho', 'carreiro'])
        previous_q = routed(-1, 'reference')+states[i-1, -1]
        origin_q = routed(0, 'reference')+local[0, -1]
        # Common interpolation to the exact anchor; origin local runoff is
        # forecast-driven, and both routed endpoints use past upstream inputs.
        reconstructed_anchor = reconstruct_anchor(previous_q, origin_q, delay_h)
        ...
        for horizon in range(1, 13):
            ...
            actual = float(d['h'][i+horizon]); predictions = {}; components = {}
            correction = residual_correction(residual, horizon+delay_h, tau_hours)
            for label, family in [('reference', 'reference'), ('julho_levels', 'level_and_slopes')]:
                uncorrected = routed(horizon, family)+local[horizon, -1]
                q = uncorrected+correction
                components[label+'_uncorrected_q_m3_s'] = float(uncorrected) if np.isfinite(uncorrected) else None
                components[label+'_corrected_q_m3_s'] = float(q) if np.isfinite(q) else None
            ...
            row = {'origin': iso(t[i]), 'target_time': iso(t[i]+horizon*3600), 'nominal_lead_h': horizon,
                   'anchor_at': iso(t[i]-delay_minutes*60), 'anchor_delay_minutes': delay_minutes,
                   'correction_tau_hours': tau_hours,
                   'anchor_residual_q_m3_s': float(residual) if np.isfinite(residual) else None,
                   'decayed_correction_q_m3_s': float(correction) if np.isfinite(correction) else None,
                   'state_rain_observations_end_at': iso(t[i]-3600),
                   'actual_m': actual if np.isfinite(actual) else None,
                   **predictions, **components, 'status': status,
                   'upstream_target_observation_missing': bool(not np.isfinite(d['julho'][i+horizon-1]))}
            if proxy is not None:
                # Count unique actually required inputs, excluding exactly-zero weights.
                required_history = set(); required_future = set()
                for lag, weight in zip(range(4, 25), weights['carreiro']):
                    if weight == 0: continue
                    required_history.update([i-1-lag, i-lag])
                    k = horizon-lag
                    if k < 0: required_history.add(i+k)
                    else: required_future.add(k)
                row['modeled_car_history_inputs'] = sum(not np.isfinite(original_car_history[j]) for j in required_history)
                row['modeled_car_future_inputs'] = sum(not np.isfinite(original_car_future[i, k]) for k in required_future)
                row['uses_missing_flow_proxy'] = bool(row['modeled_car_history_inputs'] or row['modeled_car_future_inputs'])
            result.append(row)
    ...
    dump(out/'experiment.json', {'input_sha256': input_hashes, 'status_counts': dict(Counter(r['status'] for r in result)),
                                'anchor_delay_minutes': delay_minutes,
                                'correction_tau_hours': tau_hours,
                                'skip_exact_zero_routing_weights': skip_zero,
                                'missing_flow_policy': missing_policy, 'proxy_models_fitted': len(proxy_models),
                                ...
                                'fitted_here': bool(proxy_models), 'status': 'Previously inspected historical development only',
