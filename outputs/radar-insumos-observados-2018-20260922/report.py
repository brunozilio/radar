...
c=json.loads((R/'coverage.json').read_text());v=json.loads((R/'verification.json').read_text())
txt='''# ANA observado2018 —27postos, duas janelas de10dias
...
'''
for r in c:
 h=r['fields']['NivelFinal'];rain=r['fields']['ChuvaFinal'];txt+=f"|{r['window']}|{r['station']}|{r['output_records']}|{r['modal_step_seconds']}|{h['finite_approved']}|{rain['finite_approved']}|{rain['empty']}|{rain['finite_other_qc']}|\n"
txt+='''
## Arquivos e limites
...
'''
txt+=f"Verificação final: **{v['check_count']} checks passaram**,62 hashes de fontes revisados ao final,54 novas tentativas dentro do plano,9626 registros XML representados exatamente uma vez. Manifesto próprio verifica todos os artefatos após esta auditoria.\n"
