...
ck('54 new +8 reused',len(manifest)==62 and sum(not s['reused'] for s in manifest)==54 and sum(s['reused'] for s in manifest)==8)
ck('map unchanged',sha(W/plan['weights_source'])==plan['weights_sha256'])
...
ck('54 preserved attempt records',len(list((R/'raw').glob('*.attempt.json')))==54)
...
for win in plan['windows']:
 for station in plan['stations']:
  ...
  directory=R/'stations'/win['id'];directory.mkdir(parents=True,exist_ok=True);out=directory/f'ana-{station}-all-qc.jsonl'
...
ck('54 series output',len(summaries)==54)
