"""Independent 2018 source-to-matrix audit; imports no production helpers/models."""
from pathlib import Path
import json,csv,hashlib,datetime as dt,math
import numpy as np
from rain_contract_audit import ended_overlap
R=Path(__file__).resolve().parent;W=R.parents[1];M=W/'outputs/radar-matrizes-observadas-2018-20260922';A=W/'outputs/radar-insumos-observados-2018-20260922';O=W/'outputs/auditoria-qi-radar-2018-20260922'
TZ=dt.timezone(dt.timedelta(hours=-3));checks=[];sources={}
LEVELS={'86510000':(3600,0),'86472000':(1800,900),'86472600':(900,900),'86500000':(1800,900)}
...
def main():
 ...
 protocol=read(W/'docs/radar-2018-observed-features-protocol.json');weights=read(W/'outputs/mucum-propagacao-2026-09-21/chuva-pesos.json');lags=read(W/'outputs/auditoria-latencias-chuva-20260921/latencies.json')['stations']
 ons=[]
 ...
 for win in protocol['windows']:
  ...
  for code in sorted(r['station'] for r in lags):
   p=use(A/'stations'/warm/f'ana-{code}-all-qc.jsonl');rows=[json.loads(l) for l in p.read_text().splitlines()];ts=np.array([ep(r['DataHora']) for r in rows]);ck(label+code+' chronological',np.all(np.diff(ts)>0));cache[code]=(rows,ts)
  ...
  F=np.column_stack(cols);compare(label+' all120',F,D['features'],2e-10);ck(label+' catalog',names==catalog);compare(label+' complete24',np.isfinite(F[:,:24]).all(1).astype(float),D['complete24'].astype(float));ck(label+' dH0.5 missing',np.isnan(F[:,1]).all())
  prior=dict(np.load(use(R/f'rain-reconstructed-{warm}.npz')));compare(label+' pre-matrix independent rain',prior['rain_features'],F[:,45:],2e-10)
  for j in range(120):
 ...
 ck('all coverage sizes',len(storedtargets)==24 and len(storedrain)==60 and len(storedfeatures)==240 and len(storedlevels)==1344 and len(storedqi)==6048)
