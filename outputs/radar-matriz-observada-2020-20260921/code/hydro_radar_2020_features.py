"""Prepare a 120-column observed RADAR matrix from frozen 2020 source files."""
import csv,json
...
HYDRO=ROOT/'outputs/radar-insumos-observados-2020-20260921'
QI=ROOT/'outputs/auditoria-qi-radar-2020-20260921'
LAGS=ROOT/'outputs/auditoria-latencias-chuva-20260921'
OUT=ROOT/'outputs/radar-matriz-observada-2020-20260921'
def save(name,rows):
    with (OUT/name).open('w') as f:
        w=csv.DictWriter(f,fieldnames=list(rows[0]));w.writeheader();w.writerows(rows)
def run():
    for p in (HYDRO,LAGS,QI):verify(p)
    protocol=ROOT/'docs/radar-2020-observed-features-protocol.json'
    ...
    paths=[protocol,wp,LAGS/'latencies.json',QI/'ons-references.json',QI/'audit.json',QI/'lag-source-trace.csv',Path(__file__),
    ...
    grid=np.arange(epoch('2020-06-28T00:00:00-03:00'),epoch('2020-07-21T00:00:00-03:00'),900)
    assert len(grid)==2208
    ...
    ons=[]
    for r in json.loads((QI/'ons-references.json').read_text()):
    ...
    keep=times>=epoch('2020-07-01T00:00:00-03:00')
    times,F,H,truth=times[keep],F[keep],H[keep],truth[phase][keep]
    assert F.shape==(480,120) and np.all(np.diff(times)==3600)
    ...
    for g in weights:
        for h in (1,3,6,12,24,48):
            ...
            coverage.append(dict(group=g['group'],window_h=h,origins=480,finite_precipitation=int(np.isfinite(p).sum()),
    ...
    for h in range(1,13):
        ...
        pairs.append(dict(horizon_h=h,origins=480,finite_base=int(np.isfinite(H).sum()),finite_target=int(np.isfinite(target).sum()),
    ...
    (OUT/'qi-diagnostics.json').write_bytes((QI/'audit.json').read_bytes())
    (OUT/'qi-origin-diagnostics.csv').write_bytes((QI/'origin-diagnostics.csv').read_bytes())
    paths.append(QI/'origin-diagnostics.csv')
    for p in paths:
        if p.suffix=='.py':(OUT/'code'/p.name).write_bytes(p.read_bytes())
    result=dict(input_sha256={str(p.relative_to(ROOT)):sha(p) for p in paths},origins=480,features=120,
