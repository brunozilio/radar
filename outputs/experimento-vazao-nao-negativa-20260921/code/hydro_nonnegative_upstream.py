...
def positive_ridge_fit(X,y,train,alpha,weights):
    ...
    if not solution.success or np.min(solution.x[:X.shape[1]]) < -1e-9:
        raise ValueError('Nonnegative ridge solver did not converge')
    state['beta']=solution.x
    return state,{'iterations':solution.nit,'optimality':float(solution.optimality),'success':bool(solution.success),'minimum_numeric_coefficient':float(np.min(solution.x[:X.shape[1]]))}
