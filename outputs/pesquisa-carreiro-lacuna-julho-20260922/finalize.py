...
for r in json.loads((P/'request-manifest.json').read_text()):assert sha(P/r['file'])==r['sha256']
for path,digest in json.loads((P/'reused-source-hashes.json').read_text()).items():assert sha(R/path)==digest,path
