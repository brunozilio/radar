...
OUT=ROOT/'outputs/auditoria-radar-identidade-receita-20260922-v2'
...
def main():
    ...
    for issue in issues:
        p=issue['payload'];code={};params={};model_hashes={};unsealed=[];skipped=[];other=[]
        for a in p['model_artifacts']:
            ...
            if not name.endswith(('.joblib','.py','.txt')):
                other.append(name)
                continue  # Data/state artifacts are not program code or settings.
            if 'blob' not in a:
        ...
        item=dict(receipt=issue['sha256'],issued_at=issue['recorded_at'],reference_at=p['reference_at'],model_version=p['model_version'],
            ...
            has_runtime_versions=bool(p.get('runtime_versions')),training_cutoff=p['training_cutoff'],unsealed_required_artifacts=len(unsealed),non_radar_models_skipped=len(skipped),non_code_state_artifacts_excluded=len(other))
        rows.append(item);details.append(dict(**item,recipe=recipe,model_sha256=model_hashes,unsealed_names=unsealed,non_radar_skipped_names=skipped,non_code_state_names=other));by_version[p['model_version']].append(item)
    groups=[]
