...
def same_time_observation(source,z,key,origin):
    """A finite as-of grid value alone does not establish a current reading."""
    station=key.split(':')[0]
    with (source/'idades-fontes.csv').open() as f:
        rows=[r for r in csv.DictReader(f) if r['source']==station]
    if len(rows)!=1 or not rows[0]['last_time'] or epoch(rows[0]['last_time'])!=origin:
        return float('nan')
    return float(z['raw:'+key][-1]/1000)
...
def run(source,out):
    ...
    inputs=[BASE/'dados-roteamento.npz',BASE/'telemetria-latencia.npz',source/'dados-roteamento.npz',source/'telemetria-latencia.npz',source/'idades-fontes.csv',*code]
