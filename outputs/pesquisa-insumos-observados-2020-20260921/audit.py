...
P=Path(__file__).resolve().parent;ROOT=P.parents[1];plan=json.loads((P/'collection-plan.json').read_text());lo=datetime.fromisoformat(plan['window_start']);hi=datetime.fromisoformat(plan['window_end'])+timedelta(days=1);result=[];manifest=[]
def num(v):
    try:n=float(v)
...
for code in plan['stations']:
    ...
    expected15=[lo+timedelta(minutes=15*i) for i in range(int((hi-lo).total_seconds()/900))];missing15=[t for t in expected15 if t not in set(unique)];entry['quarter_hour_timestamp_slots']=int((hi-lo).total_seconds()/900);entry['quarter_hour_grid_absent_count']=len(missing15);entry['quarter_hour_gaps']=intervals(missing15,900);entry['quarter_hour_count_is_not_universal_rain_denominator']=True
