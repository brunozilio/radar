...
def import_saved(root, radar_dir, hge_dir):
    radar = json.loads((radar_dir/'previsao-atualizada.json').read_text())
    hge = json.loads((hge_dir/'resultado.json').read_text())
    for model_id, source, metadata, produced, points in [
        ...
        ('hge_arno_reference',hge_dir/'resultado.json',hge,hge['generated_at'],
         [{'valid_at':p['time'],'nominal_lead_h':p['lead_h'],'level_m':p['hge_reference_m']} for p in hge['forecast']]),
    ]:
...
def evaluate(root, at=None, records=None, observations=None):
    ...
    for record in records:
        ...
        for point in p['points']:
            ...
            row = {'forecast_record':record['sha256'],'station_id':p['station_id'],'datum_id':p['datum_id'],'model_id':p['model_id'],'model_version':p['model_version'],
...
def report(root):
    ...
    pairs = evaluate(root,at,records,observations)
    floods=flood_event_audit(records,observations,pairs,at)
    out = root/'reports'/at.strftime('%Y%m%dT%H%M%S%fZ')
    out.mkdir(parents=True, exist_ok=False)
    (out/'code').mkdir()
    report_code=[]
    for source in [Path(__file__),ROOT/'scripts/hydro_cadence.py',ROOT/'scripts/hydro_flood_events.py']:
    ...
    for key, group in sorted(groups.items()):
        hits=sum(p['hit_within_050m'] for p in group)
        score.append({'model_id':key[0],'version':key[1],'lead_h':key[2],'regime':key[3],
                      'n':len(group),'hits':hits,'accuracy_percent':100*hits/len(group)})
...
if __name__=='__main__':
    ...
    imp=commands.add_parser('import-saved');imp.add_argument('--radar',type=Path,required=True);imp.add_argument('--hge',type=Path,required=True)
    ...
    elif args.command=='import-saved':import_saved(args.ledger,args.radar,args.hge)
