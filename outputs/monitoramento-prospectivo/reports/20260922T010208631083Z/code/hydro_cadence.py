...
MODELS=['radar_arvores_live_candidate']
def stamp(value):
...
def audit(records,at):
    ...
    transitions=sorted((r for r in records if r['kind']=='cadence_model_transition'), key=lambda r:stamp(r['payload']['effective_from']))
    previous_effective=start
    for transition in transitions:
        effective=stamp(transition['payload']['effective_from'])
        if effective<stamp(transition['recorded_at']) or effective<=previous_effective or (effective-start).total_seconds()%3600:
            raise ValueError('Model transition must be preregistered at a future hourly boundary')
        if transition['payload']['expected_models']!=MODELS:
            raise ValueError('Only the explicit Radar-only transition is supported')
    ...
    while cursor+timedelta(hours=1)<=at:
        ...
        active_transition=next((r for r in reversed(transitions) if stamp(r['payload']['effective_from'])<=cursor),None)
        expected=set(active_transition['payload']['expected_models'] if active_transition else p['expected_models'])
        ...
        windows.append({'start':cursor.isoformat(),'end_exclusive':end.isoformat(),'status':status,'expected_models':sorted(expected),'model_transition_record':active_transition['sha256'] if active_transition else None,'attempts':details})
        cursor=end
