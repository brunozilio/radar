...
import argparse, ast, csv, hashlib, importlib.util, json, os, sys
...
def calculate(out,root,reference,receipts):
    ...
    F=np.column_stack(fc);future_q={};alphas={(r['source'],int(r['lead_h'])):float(r['alpha']) for r in rows(BASE/'previsao-vazoes-montante.csv')}
    ...
    code=[Path(__file__),ROOT/'scripts/hydro_latency_forecast.py',ROOT/'scripts/hydro_mass_check.py',HGE/'run.py',HGE/'vendor/hydrological_model.py']
