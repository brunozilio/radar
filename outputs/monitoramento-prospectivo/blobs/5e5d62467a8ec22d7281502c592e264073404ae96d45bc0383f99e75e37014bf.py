...
def calculate(out,root,reference,receipts,cycle_id=None):
    ...
    for source,key in [('julho','julho:Q'),('carreiro','86500000:Q')]:
        ...
        for k in range(14):
            ...
            future_q[source][k]=float(max(0,ridge_predict(state,F[-1])+known[-1])*1000)
    ...
    for model in MODELS:
        ...
        weather_source=Path(os.environ.get('HYDRO_HISTORY_DIR',str(out/'raw')))/f'weather-{model}.json'
        live=weather_values(weather_source,0,alltimes,'precipitation')
        ...
        # Cumulative snapshots preserve earlier collected estimates beyond the
        # rolling seven-day endpoint window. A revised null remains unknown.
        weather_times=json.loads(weather_source.read_text())[0]['hourly']['time']
        covered=(alltimes>=epoch(weather_times[0]))&(alltimes<=epoch(weather_times[-1]))
        a[covered]=live[covered];members.append(a)
    ...
    code=[Path(__file__),*[ROOT/'scripts'/name for name in ['hydro_latency_forecast.py','hydro_mass_check.py','hydro_rain_windows.py','hydro_routing_data.py','hydro_model.py','hydro_routing_fit.py','hydro_flow_diagnostics.py','hydro_history.py','hydro-hourly-requirements.txt']],out/'upstream-extrapolation.json',HGE/'run.py',HGE/'vendor/hydrological_model.py']
...
def run():
    ap=argparse.ArgumentParser();ap.add_argument('--source',type=Path);ap.add_argument('--ledger',type=Path,default=ledger.DEFAULT);ap.add_argument('--weather-transition',type=Path);args=ap.parse_args()
...
def execute(args):
    ...
    with threadpool_limits(limits=2): result=calculate(out,args.ledger,reference,receipts,args.cycle_sha256)
