...
def run():
    ...
    if args.revision_reason:
        if not {'radar_arvores_live_candidate','hge_arno_live_candidate'}<=completed:
            raise ValueError('Manual revision requires both original hourly forecasts')
    ...
    if {'radar_arvores_live_candidate','hge_arno_live_candidate'}<=completed and not args.revision:
        lock.close()
