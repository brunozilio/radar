...
s=json.loads((R/'station-summary.json').read_text());v=json.loads((R/'verification.json').read_text());f=json.loads((R/'frozen-observed-verification.json').read_text());sources=json.loads((R/'source-index.json').read_text())
txt='''# Proveniência/QC recente para comparação experimental com2018
...
'''
for r in s:
 if r['station'] in ['86510000','86472000','86472600','86500000']:txt+=f"|{r['station']}|{r['records']}|{r['fields']['level']['legacy_finite']}|{r['frozen_level_delay_seconds']/60:g}min|\n"
txt+='''
...
'''
txt+=f"Auditoria principal:{v['check_count']} checks passaram; suplemento120colunas exato, vínculo de{f['prior_rainfall_source_hashes_verified']} insumos da auditoria dechuva. Todos os arquivos são experimentais, sem promoção nem alegação de98%. Manifesto final/check preserva esta pasta.\n"
(R/'README.md').write_text(txt)
