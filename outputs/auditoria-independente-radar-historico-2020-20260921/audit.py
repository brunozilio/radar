...
import numpy as np
OUT=Path(__file__).resolve().parent;ROOT=OUT.parents[1];E=ROOT/'outputs/experimento-radar-historico-2020-20260921';sources={};checks=[]
...
def readbytes(p):sources[str(p.relative_to(ROOT))]=sha(p);return p.read_bytes()
...
member=json.loads(readbytes(OUT/'membership-verification.json'));actualm=dict(np.load(E/'training-masks.npz'));expectedm=dict(np.load(OUT/'expected-training-masks.npz'))
ok('36maskkeys',len(actualm)==36 and set(actualm)==set(expectedm))
for k in expectedm:ok('actualmask:'+k,np.array_equal(actualm[k],expectedm[k]))
training=rows(E/'training.csv')
for r in training:
 expected=next(x for x in member['training'] if x['phase']==r['phase'] and x['horizon_h']==int(r['horizon_h']))
 for k in ['original_n','added_n','n','original_targets_ge_7m','added_targets_ge_7m','added_response_min_m','added_response_max_m','leaf_nodes']:
  assert math.isclose(float(r[k]),expected[k],abs_tol=1e-12,rel_tol=0),(r['phase'],r['horizon_h'],k)
 for k in ['latest_training_target','cutoff_exclusive','loss']:assert r[k]==expected[k]
 ok('training:'+r['phase']+r['horizon_h'],True)
for path,h in member['source_sha256'].items():ok('membership_source:'+path,hashlib.sha256((ROOT/path).read_bytes()).hexdigest()==h)
for path in [E/'training-masks.npz',OUT/'expected-training-masks.npz']:readbytes(path)
manifest=json.loads(readbytes(E/'artifact-hashes.json'))
for r in manifest:
 if r['file'] in ['predictions.csv','evaluation.csv','training.csv','training-masks.npz','protocol.json']:ok('experiment_hash:'+r['file'],hashlib.sha256((E/r['file']).read_bytes()).hexdigest()==r['sha256'])
ok('membershippassed',member['passed'] and not member['reserved_windows_used'] and member['eligible_zero_qi_pairs_all_horizons']==0)
...
worst=max(extreme,key=lambda r:r['augmented_abs_error_m'])
report=dict(passed=True,checks=checks,membership_checks_passed=len(member['checks']),training_weight_reconstruction=member['training'],membership_limits=member['limits'],worst_test_h12=worst,metrics=metrics,comparisons=comparisons,tradeoffs=trades,extreme_response_summary=exsum,extreme_response_cases=extreme,failures=failures,limits=['Development data already inspected; not independent holdout.','Errors use finite pairs; hit fraction over observed targets retains failures.','No fits or model inspection in this audit.','Mixed changes do not establish causal attribution or promotion.'])
...
lines += ['', 'As 92 verificações independentes de membership passaram; as 36 máscaras salvas e as 24 linhas de treinamento concordam com a reconstrução. Somente 2020 foi acrescentado aos dados originais de 2025/26: nenhum dado de 2021/22 reservado ou de 2023/24 foi incluído. Cutoffs estritos dos alvos: 01/10/2025 e 01/07/2026.', '', 'Pesos reconstruídos pela fórmula congelada 1 + 2·I(|alvo−base|≥1 m) + 2·I(alvo≥9 m). São somas derivadas das memberships e da fórmula, não um novo ajuste nem um log de pesos do estimador.', '', '| Fase | h | Pares de 2020 | Alvos ≥7 m | Peso original | Peso 2020 | Fração do peso de 2020 |', '|---|---:|---:|---:|---:|---:|---:|']
for t in member['training']:
 if t['horizon_h'] in (1,6,12): lines.append(f"| {t['phase']} | {t['horizon_h']} | {t['added_n']} | {t['added_targets_ge_7m']} | {t['original_weight']} | {t['added_weight']} | {100*t['weight_fraction_2020']:.4f}% |")
lines += ['', 'As 23 origens com algum Q/I zero não geram nenhum par elegível em h1..12: 22 têm base Muçum ausente; a única base finita é 08/07/2020 05h, 19,02 m, com todos os 12 alvos ausentes. Isto decorre das máscaras existentes, sem filtro novo sobre zeros; o apagão/QC do pico continua sendo uma limitação.', '', f"Pior erro do candidato no teste/12h: origem {worst['origin']}, alvo {worst['target_time']}, base {worst['base_m']:.2f} m, observado {worst['actual_m']:.2f} m, previsão {worst['augmented_m']:.9f} m, erro absoluto {worst['augmented_abs_error_m']:.9f} m. Na mesma origem, controle {worst['control_m']:.9f} m e erro {worst['control_abs_error_m']:.9f} m. O máximo do controle (7,589996584 m) ocorre em outra origem.", '', 'No teste, MAE melhora nos 12 horizontes e acertos melhoram em 11 (um empate). Na validação, acertos melhoram em três, empatam em cinco e pioram em quatro; h6 cai de 24 para 21/28. Não há dominância geral.']
