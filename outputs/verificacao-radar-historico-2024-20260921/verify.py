"""Audit frozen 2024 augmentation without refitting or touching live forecasts."""
import csv
import hashlib
import json
import sys
...
def main():
    ...
    d=dict(np.load(REF/'features.npz')); n=dict(np.load(NEW/'features.npz'))
    ...
    for a,b in zip(preds,reference):
        ...
        assert bool(a['native_control_m'])==bool(a['augmented_m'])
    info={(r['phase'],int(r['horizon_h'])):r for r in rows(RUN/'training.csv')}
    oldinfo={(r['phase'],int(r['horizon_h'])):r for r in rows(REF/'training.csv') if r['family']=='candidate'}
    masks=dict(np.load(RUN/'training-masks.npz'))
    start=epoch('2025-10-01T00:00:00-03:00'); end=epoch('2026-07-01T00:00:00-03:00')
    boundary=epoch('2024-05-02T00:00:00-03:00')
    checks=[]
    for phase in ('validation','test'):
        for h in range(1,13):
            ...
            assert int(newmask.sum())==int(meta['added_n'])==744-h
            ...
            assert model.n_features_in_==180 and model.n_iter_==180
    ...
    for r in rows(REF/'evaluation.csv'):
        ...
        translated=dict(r,family='native_control'); current=metrics[metric_key(translated)]
    ...
    for phase in ('validation','test'):
        for h in range(1,13):
            for subset in ('all','level_ge_7m'):
                a=metrics[phase,str(h),subset,'full_schedule','native_control']
    ...
    lines=['# RADAR: acréscimo de histórico de2024','',
    ...
    for phase in ('validation','test'):
        for h in (1,6,12):
            for family,label in [('native_control','Referência180'),('augmented','+2024')]:
    ...
    lines+=['','## Dados e limites','',
        f"Em12h entram{tr['added_n']} pares, dos quais{tr['added_targets_ge_7m']} com alvo≥7m. A maior resposta no treino passa de{float(tr['original_response_max_m']):.2f}m para{float(tr['combined_response_max_m']):.2f}m. Não há seleção dos exemplos extremos: o mês inteiro elegível foi incluído.", '',
