"""Audit gauge/rain/year factorial metrics and predictions without refitting."""
import csv
import hashlib
import json
import sys
...
RUN=ROOT/'outputs/experimento-radar-factorial-blocos-20260921'
REF=ROOT/'outputs/experimento-radar-core33-20260921'
...
def main():
    ...
    columns={'levels45':np.arange(45),'rain108':np.r_[0:12,24:120]}
    families=('observed_control','augmented','core_original','core_augmented','levels45_original','levels45_augmented','rain108_original','rain108_augmented')
    ...
    for a,b in zip(predictions,previous):
        ...
        assert all(bool(a[family+'_m'])==bool(a['observed_control_m']) for family in families)
    ...
    for phase in ('validation','test'):
        for h in range(1,13):
            ...
            for family in ('levels45_original','levels45_augmented','rain108_original','rain108_augmented'):
                meta=info[phase,h,family];original=oldinfo[phase,h];include=family.endswith('_augmented');group=family.split('_')[0]
                ...
                actual=model.predict(F[indices][:,columns[group]])+base[indices]
                expected=np.array([float(r[family+'_m']) for r in selected])
                np.testing.assert_array_equal(actual,expected)
                assert model.n_features_in_==len(columns[group]) and model.n_iter_==180
    ...
    for phase in ('validation','test'):
        for h in range(1,13):
            for subset in ('all','level_ge_7m'):
                # All edges of the2x2x2 design: year, auxiliary levels, rain.
                for a,b in [('observed_control','augmented'),('core_original','core_augmented'),
                            ('levels45_original','levels45_augmented'),('rain108_original','rain108_augmented'),
                            ('core_original','levels45_original'),('core_augmented','levels45_augmented'),
                            ('rain108_original','observed_control'),('rain108_augmented','augmented'),
                            ('core_original','rain108_original'),('core_augmented','rain108_augmented'),
                            ('levels45_original','observed_control'),('levels45_augmented','augmented')]:
    ...
    lines=['# RADAR: comparação dos blocos de chuva e níveis auxiliares','',
