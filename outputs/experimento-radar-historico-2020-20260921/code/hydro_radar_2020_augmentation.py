"""Frozen120-column RADAR control versus predeclared2020 data augmentation."""
import csv
...
def run():
    ...
    protocol = ROOT/'docs/radar-2020-augmentation-protocol.json'
    ...
    qi_path=verified(new,'qi-origin-diagnostics.csv');paths.append(qi_path)
    qi=list(csv.DictReader(qi_path.open()));assert len(qi)==len(nt)
    assert all(epoch(r['origin'])==v for r,v in zip(qi,nt))
    zeros=np.array([r['uses_any_zero']=='True' for r in qi])
    reservation=ROOT/'docs/radar-historical-evaluation-reservation.json';paths.append(reservation)
    for window in json.loads(reservation.read_text())['windows']:
        lo,hi=epoch(window['start_inclusive_local']),epoch(window['stop_exclusive_local'])
        assert not ((nt>=lo)&(nt<hi)).any() and not ((t>=lo)&(t<hi)).any()
    F = F[:,:120]
    assert F.shape == (len(t),120) and NF.shape == (480,120)
    ...
    out = ROOT/'outputs/experimento-radar-historico-2020-20260921'
    ...
    assert nt[0]==epoch('2020-07-01T00:00:00-03:00') and nt[-1]==boundary-3600
    ...
    for h in range(1,13):
        ...
        print('completed RADAR2020 observed augmentation horizon',h,flush=True)
