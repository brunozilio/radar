"""Independent masking experiment audit; inference only, no fitting/network."""
...
P=Path(__file__).resolve().parent;R=P.parents[1];E=R/'outputs/experimento-radar-ausencia-carreiro-20260922';B=R/'outputs/experimento-radar-mistura-atrasos-20260922'
F=('profile_A_control','profile_B_control','mixed_profile_candidate','carreiro_missing_exposure');checks=[];inputs={}
...
def main():
 ...
 ck('protocol hash',sha(E/'protocol.json')=='5360df29a1c77e5b4bf69a21f56dddf2530428e928c299f7d69b444b8caf4aeb')
 ...
 for meta in (pre,exp):
  ...
  ck('prefit maskingplan',sha(track(E/'masking-plan.npz'))==meta['masking_plan_sha256'])
 for folder in (B,E):
  for r in js(folder/'artifact-hashes.json'):ck('artifact '+folder.name+'/'+r['file'],sha(track(folder/r['file']))==r['sha256'])
 ck('executed code identical',sha(E/'code/hydro_radar_missing_exposure.py')==sha(track(R/'scripts/hydro_radar_missing_exposure.py')))
 d=nz(B/'prepared-inputs.npz');masks=nz(B/'training-masks.npz');plan=nz(E/'masking-plan.npz');t=d['times'];truth=d['truth'];n=len(t);assign=d['assignment']
 ck('PCG57 assignment',eq(assign,np.random.Generator(np.random.PCG64(57)).integers(0,2,size=n,dtype=np.int8)))
 masked=np.random.Generator(np.random.PCG64(58)).random(n)<.25
 ck('PCG58 selection3274',eq(masked,plan['masked']) and eq(t,plan['times']) and int(masked.sum())==pre['selected_timeline_origins']==3274 and n==pre['timeline_origins']==12912)
 X=d['features_A'].copy();X[assign==1]=d['features_B'][assign==1];base=d['base_A'].copy();base[assign==1]=d['base_B'][assign==1]
 modified=X.copy()
 for col in range(18,24):modified[masked,col]=np.nan
 ck('only specified block',eq(modified[:,:18],X[:,:18]) and eq(modified[:,24:],X[:,24:]) and eq(modified[~masked],X[~masked]) and np.isnan(modified[masked,18:24]).all())
 ...
 for h in range(1,13):
  ...
  for phase,left,right in (('validation',start,end),('test',end,stop)):
   ...
   ck(f'{phase}{h}mask',eq(mask,masks[f'{phase}_h{h}']));ix=np.flatnonzero(mask);y=target[ix]-base[ix];w=1+2*(abs(y)>=1)+2*(target[ix]>=9);selected=masked[ix];m=int(selected.sum());info=train[phase,h]
   ck(f'{phase}{h}order/cutoff',np.all(np.diff(ix)>0) and len(np.unique(t[ix]))==len(ix) and np.all(t[ix]+h*3600<left))
   ck(f'{phase}{h}parent fields',all(info[k]==v for k,v in oldtrain[phase,h].items() if k!='family') and int(info['n'])==len(ix) and int(info['weights_sum'])==int(sum(w)))
   ck(f'{phase}{h}mask cells',np.isfinite(X[ix,:24]).all() and np.isnan(modified[ix,18:24]).sum()==6*m and eq(modified[ix][~selected],X[ix][~selected]))
   calculated=dict(masked_origins=m,unmasked_origins=len(ix)-m,cells_made_missing=6*m,masked_high_targets=int((selected&(target[ix]>=7)).sum()),unmasked_high_targets=int((~selected&(target[ix]>=7)).sum()))
   ck(f'{phase}{h}mask counts',all(int(info[k])==v for k,v in calculated.items()))
   ...
   training.append(dict(phase=phase,horizon_h=h,rows=len(ix),weights_sum=int(sum(w)),**calculated,indices_sha256=hashlib.sha256(ix.tobytes()).hexdigest(),responses_sha256=hashlib.sha256(y.tobytes()).hexdigest(),weights_sha256=hashlib.sha256(w.tobytes()).hexdigest(),reconstructed_masked_inputs_sha256=hashlib.sha256(modified[ix].tobytes()).hexdigest()))
   for profile in ('A','B'):
 ...
 dump('verification.json',dict(passed=True,checked_at_utc=datetime.now(timezone.utc).isoformat(),checks_count=len(checks),checks=checks,models=24,control_applications=144,candidate_applications=48,metrics=1152,rows=204168,selected_origins=3274,timeline_origins=12912,fit_or_network=False,limits=['Serialized HGB does not prove the samples/weights actually consumed during fit; reconstruction, saved plan, executed source and hashes provide bounded evidence, no refit.','Independent random origin masking does not reproduce actual outages or informative missingness.','Same development periods already inspected; profiles are paired views.','Recorded local timestamps are not authenticated independent proof of fit timing.','No promotion or operational change.']))
